        ...
        if (hitPoint) {
            ...
            if (enemy) {
                ...
                if (this.particleSystem) {
                    this.particleSystem.emitBlood(hitPoint, bloodDirection, 12);
                }
            } else {
                if (this.particleSystem) {
                    if (!hitObject) {
                        // Ground raymarch hit
                        this.particleSystem.emitDust(hitPoint, hitNormal ?? this.tmpUp, 14);
                    } else {
                        const ud = getUserData(hitObject);
                        if (ud.isTree) this.particleSystem.emitDebris(hitPoint, hitNormal ?? this.tmpUp, 16);
                        else if (ud.isGrass) this.particleSystem.emitDust(hitPoint, hitNormal ?? this.tmpUp, 12);
                        else if (ud.isRock) this.particleSystem.emitSparks(hitPoint, hitNormal ?? this.tmpUp, 12);
                        else this.particleSystem.emitSparks(hitPoint, hitNormal ?? this.tmpUp, 10);
                    }
                }
            }
        }
